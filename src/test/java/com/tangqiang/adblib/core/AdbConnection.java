...
public class AdbConnection implements Closeable {
            ...
            public void run() {
                ...
                while (!connectionThread.isInterrupted()) {
                    try {
                        ...
                        AdbMessage msg = AdbMessage.parseAdbMessage(inputStream);

                        /* Verify magic and checksum */
                        if (!AdbProtocol.validateMessage(msg))
                            continue;
                        AdbCommand command = AdbCommand.fromValue(msg.command);
                        logger.debug("Read a message:" + command + "(" + msg.command + ") " + msg);
                        switch (command) {
                            ...
                            case AUTH:
                                ...
                                if (msg.arg0 == AdbProtocol.AUTH_TYPE_TOKEN) {
                                    ...
                                    logger.debug("Send a package: AUTH");
                            ...
                            default:
                                ...
                                logger.warn("Unrecognized packet ");
                                break;
                        }
                    } catch (Exception e) {
                ...
                logger.info("ConnectionThread end ...");
            }
        });
    }
    ...
    public void connect() throws IOException, InterruptedException {
        ...
        byte[]  packa = AdbProtocol.generateConnect();
        logger.debug("Send a package: Connect");
        outputStream.write(packa);
        outputStream.flush();
    ...
    public void execute(String command) throws IOException, InterruptedException {
        logger.info("ADB execute:" + command);
        ...
        /* Send the open */
        byte[] pack = AdbProtocol.generateWrite(localId, 0, command);
        logger.debug("Send a package: Write");
    ...
    public AdbStream open(String destination) throws UnsupportedEncodingException, IOException, InterruptedException {
        ...
        /* Send the open */
        byte[] pack = AdbProtocol.generateOpen(localId, destination);
        logger.debug("Send a package: Open - " + destination + " localId:" + localId);
        outputStream.write(pack);
        outputStream.flush();
